...
public class TitleManager extends JFrame implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if (e.getActionCommand().equals("수정")) {
					...
					pContent.setTitle(upTitle); // 텍스트필드에 선택된 객체 가져오기
				}
				// 동일직책 사원보기
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			if (e.getSource() == btnAdd) {
				if (btnAdd.getText().equals("추가")) { // 버튼 이름이 '추가' 일때
	...
	private void actionPerformedBtnUpdate(ActionEvent e) {
		Title newTitle = pContent.getTitle();	//텍스트필드에 작성된 텍스트를 이용해 객체를 생성해서 반환받는다.
		service.modifyTitle(newTitle);	// 수정하는 쿼리문을 실행하는 메소드를 호출한다.
	...
	protected void actionPerformedBtnAdd(ActionEvent e) {
		Title title = pContent.getTitle();	//텍스트필드에 작성된 텍스트를 가져와서 객체 생성해서 반환받는다.
